...
.PHONY: check inventory inventory-router inventory-nmac backup-routeros backup-opentofu backup-app backup-apps review-routeros ansible-check ansible-bootstrap ansible-storage ansible-k3s ansible-maintenance
...
ansible-check:
	...
	@$(ANSIBLE_PLAYBOOK) -i $(ANSIBLE_INVENTORY) ansible/playbooks/maintenance.yml --syntax-check

ansible-maintenance:
	@$(ANSIBLE_PLAYBOOK) -i $(ANSIBLE_INVENTORY) ansible/playbooks/maintenance.yml $(ANSIBLE_FLAGS)
